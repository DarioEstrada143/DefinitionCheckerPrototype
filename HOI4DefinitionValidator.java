...
/**
 * HOI4DefinitionValidator.java
 * ================================
 * Class library for the HOI4 definition.csv validator.
 * Contains all data models, parsing, checking, and reporting logic.
 *
 * Classes:
 *   IssueType         — Enum: ERROR or WARNING
 *   ValidationIssue   — A single problem found during validation
 *   Province          — Data model for one parsed definition.csv row
 *   ParseResult       — Container returned by DefinitionParser
 *   DefinitionParser  — Reads CSV into Province objects
 *   DefinitionChecker — Runs all validation logic; builds error TreeMap
 *   ReportWriter      — Outputs results to console and .txt report file
 *
 * Entry point is in Main.java.
 */
...
//  Holds the parsed Province list plus file-level metadata.
// =============================================================================

class ParseResult {
    private final List<Province>        provinces;
    private final List<ValidationIssue> parseIssues;
...
class DefinitionParser {
    ...
    private List<Province> parseLines(String[] lines) {
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            // Trailing whitespace in first 8 columns
            for (int c = 0; c < 8; c++) {
...
//  Runs all validation passes against a ParseResult.
//  Each check is its own method for clarity and extensibility.
//
//  Also builds a TreeMap<Integer, List<ValidationIssue>> keyed by province ID
//  so callers can retrieve all issues sorted in province-ID order.
//  Global issues (line 0) are stored under key -1.
// =============================================================================
...
class DefinitionChecker {
    ...
    /**
     * TreeMap of province ID → issues for that province, sorted numerically.
     * Key -1 holds global/file-level issues not tied to a specific province.
     */
    private final TreeMap<Integer, List<ValidationIssue>> errorMap = new TreeMap<>();
    ...
    /** Runs every check and returns the full issue list. */
    ...
    /**
     * Returns the TreeMap of province ID → issues, sorted numerically.
     * Must be called after runAllChecks().
     * Key -1 = global issues not tied to a specific province ID.
     */
    TreeMap<Integer, List<ValidationIssue>> getErrorMap() {
        return errorMap;
    }
    ...
    private void checkProvinceId(Province p) {
        ...
        if (p.getId() == 0)
            addError(p.getLineNumber(), "Province ID is 0. IDs must start at 1 (0 is reserved).");
        if (p.getId() < 0)
    ...
    private void checkCoastalField(Province p) {
        ...
        if ((p.isType("sea") || p.isType("lake"))
            ...
            addWarning(p.getLineNumber(), "Province type is '" + p.getRawType()
                + "' but is_coastal is true. Sea/lake provinces are not normally coastal.");
    }
    ...
    private void checkIdUniquenessAndSequence() {
        ...
        if (validIds.get(0) != 1)
            addError(0, "Province IDs do not start at 1. Lowest ID found: " + validIds.get(0));
        ...
        if (!gaps.isEmpty()) {
            String gapStr = gaps.size() < 50
                ? gaps.toString()
                : gaps.subList(0, 50) + " ... (" + gaps.size() + "+ gaps)";
    ...
    /**
     * Builds the errorMap after all checks have run.
     * Each issue is bucketed by its province ID.
     * Issues not tied to a specific province ID go under key -1 (global).
     */
    private void buildErrorMap() {
        // Build a line-number → province-ID lookup from the parsed provinces
        Map<Integer, Integer> lineToId = new HashMap<>();
        for (Province p : result.getProvinces()) {
            if (p.getId() != null && p.getId() > 0) {
                lineToId.put(p.getLineNumber(), p.getId());
            }
        }

        for (ValidationIssue issue : issues) {
            int lineNum  = issue.getLineNumber();
            Integer provId = lineToId.get(lineNum);

            // Use province ID as key if we can resolve it; otherwise -1 (global)
            int mapKey = (provId != null) ? provId : -1;

            errorMap.computeIfAbsent(mapKey, k -> new ArrayList<>()).add(issue);
        }
    }
...
//  Also prints and writes the TreeMap error summary.
// =============================================================================
...
class ReportWriter {
    ...
    private final TreeMap<Integer, List<ValidationIssue>>  errorMap;
    private final long                                     errorCount;
    private final long                                     warningCount;

    ReportWriter(Path csvPath, ParseResult result,
                 List<ValidationIssue> issues,
                 TreeMap<Integer, List<ValidationIssue>> errorMap) {
        this.csvPath      = csvPath;
        this.result       = result;
        this.issues       = issues;
    ...
    void printToConsole() {
        ...
        if (issues.isEmpty()) {
            ...
            System.out.println("── All Issues (in file order) ─────────────────────");
            for (ValidationIssue issue : issues) {
    ...
    void writeReportFile() {
        ...
        try (PrintWriter pw = new PrintWriter(
            ...
            if (issues.isEmpty()) {
                ...
                pw.println("All Issues (in file order)");
                pw.println("--------------------------");
                for (ValidationIssue issue : issues) {
                    pw.println(issue);
                }
                pw.println();
                writeErrorMapToReport(pw);
            }
    ...
    private void printErrorMap() {
        ...
        System.out.println();
        System.out.println("── Provinces With Issues (sorted by ID) ───────────");

        // Print global issues first (key = -1)
        if (errorMap.containsKey(-1)) {
            System.out.println("  [GLOBAL / file-level issues]");
            for (ValidationIssue issue : errorMap.get(-1)) {
                System.out.println("    " + issue);
            }
            System.out.println();
        }

        // Print province-specific issues in ascending ID order
        for (Map.Entry<Integer, List<ValidationIssue>> entry : errorMap.entrySet()) {
            if (entry.getKey() == -1) continue; // already printed above
            System.out.printf("  Province ID %d (%d issue(s)):%n",
                entry.getKey(), entry.getValue().size());
            for (ValidationIssue issue : entry.getValue()) {
                System.out.println("    " + issue);
            }
            System.out.println();
        }
    ...
    private void writeErrorMapToReport(PrintWriter pw) {
        if (errorMap.isEmpty()) return;

        pw.println("Provinces With Issues (sorted by Province ID)");
        pw.println("---------------------------------------------");

        if (errorMap.containsKey(-1)) {
            pw.println("[GLOBAL / file-level issues]");
            for (ValidationIssue issue : errorMap.get(-1)) {
                pw.println("  " + issue);
            }
            pw.println();
        ...
        for (Map.Entry<Integer, List<ValidationIssue>> entry : errorMap.entrySet()) {
            ...
            pw.printf("Province ID %d (%d issue(s)):%n",
                entry.getKey(), entry.getValue().size());
            for (ValidationIssue issue : entry.getValue()) {
                pw.println("  " + issue);
            }
